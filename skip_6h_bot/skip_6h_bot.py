...
image_hint = "Skip 6h Play Button"
...
def bot_cycle():
    while True:
        if locate_and_click(folder_location + 'out-of-ads.png', "Out of Ads found", click=False):
                ...
                continue
        elif locate_and_click(folder_location + 'place.png', 'Place Button found', click=False):
                locate_and_click(folder_location + 'close.png', 'Close Button found')
                continue
        tasks = [
            (folder_location + 'skip-6h-play-button.png', 'Skip 6h Play Button found'),
            (folder_location + 'skip-6h.png', 'Skip 6h Text Button found'),
        ]
        for task in tasks:
...
while confirmation == 'yes':
    try:
        position = pyautogui.locateOnScreen(folder_location + 'skip-6h-play-button.png', confidence=0.8)
        if position is not None:
